...
$temp->setReplace("{pagename}", ' <img src="./image/next.png" border="0" align="absmiddle">ประวัติการพิมพ์แท็ก Improvement');
$temp->setReplace("{content}", $temp->getTemplate("./template/tag_history_improvement.html"));
...
if (isset($rs[1]["qty_issued"])) $qty_issued = total_format($rs[1]["qty_issued"]);
$qty_issued = 0;
...
for($i=1; $i<=$rs[0][0]; $i++) {
	...
	if ($rs[$i]["uf_grade"] == "A") {
		if ($rs[$i]["active"]) $da[$lota[0]] = $i;
	}
	if ($rs[$i]["uf_grade"] == "B") {
		if ($rs[$i]["active"]) $db[$lota[0]]=$i;
	}
	if ($rs[$i]["uf_grade"] == "C") {
		if ($rs[$i]["active"]) $dc[$lota[0]]=$i;
	}
	if ($rs[$i]["uf_grade"] == "R") {
		if ($rs[$i]["active"]) $dr[$lota[0]]=$i;
	}
}
$qty1_a = 0;
$qty2_a = 0;
