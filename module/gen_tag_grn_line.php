...
$temp->setReplace("{pagename}", ' <img src="./image/next.png" border="0" align="absmiddle">gen_tag_grn_line');
$temp->setReplace("{content}", $temp->getTemplate("./template/gen_tag_grn_line.html"));

$cSql = new SqlSrv();
$grn_num = $grn_num;
$jobm = explode("+", $jobno);
$sql = "select * FROM grn_line_mst where grn_num = '$grn_num' ";
...
$sql = "select item,* FROM grn_line_mst LEFT JOIN poitem_mst on grn_line_mst.po_line = poitem_mst.po_line and grn_line_mst.po_num = poitem_mst.po_num "
        . " where grn_num = '$grn_num' ";;
$rs = $cSql->SqlQuery($conn, $sql);
echo 5555;
echo $sql;
for ($i = 1; $i <= $rs[0][0]; $i++) {
...
for ($i = $rs[0][0]; $i >= 1; $i--) {
    ...
    $list .= '<tr bgcolor="' . $bg_color . '" class="' . $cfont . '">
        ...
        <td align="center">' . $rs[$i]["grn_line"] . '</td>
