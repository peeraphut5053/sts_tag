...
$temp->setReplace("{pagename}", ' <img src="./image/next.png" border="0" align="absmiddle">ประวัติการพิมพ์แท็ก Tag C');
$temp->setReplace("{content}", $temp->getTemplate("./template/tag_history_tag_c.html"));
...
$sql = "select *, CONVERT(VARCHAR(16), mfg_date, 120)  AS mfg_date from Mv_Bc_tag where ltrim(job) = '" . $jobm[0] . "' and suffix = '" . $jobm[1] . "' and oper_num = '" . $jobm[2] . "' order by uf_pack, uf_grade;";
$rs = $cSql->SqlQuery($conn, $sql);
for ($i = 1; $i <= $rs[0][0]; $i++) {
