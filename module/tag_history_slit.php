...
$temp->setReplace("{pagename}", ' <img src="./image/next.png" border="0" align="absmiddle">ประวัติการพิมพ์แท็กสลิต');
$temp->setReplace("{content}", $temp->getTemplate("./template/tag_history_slit.html"));
...
//print_r($rs);
//print qty_mat 
$sql = "select CONVERT(DECIMAL(10,0), SUM( mat.qty)) as qty_mat from matltran_mst mat WHERE   1=1 AND  ref_num like '$jobm[0]'  AND  trans_type like '%I%'   ";
$rs2 = $cSql->SqlQuery($conn, $sql);
...
for ($i = $rs[0][0]; $i >= 1; $i--) {
    ...
    $list .= '<tr bgcolor="' . $bg_color . '" class="' . $cfont . '">
        ...
        <td>' . $rs[$i]["id"] . '</td>
        <td>' . $rs[$i]["lot"] . '</td>
		<td align="center">' . $rs[$i]["sts_no"] . '</td>
