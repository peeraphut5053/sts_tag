...
if ($rs[0][0]) {
	if ($rs[1]["end_time"]) {
		$sql = "insert into jobtran_mst (site_ref, job, trans_type, trans_date, oper_num, emp_num, shift, pay_rate, pr_rate, job_rate, start_time, wc) values ('".$site_ref."', '".sprintf("%' 10s",$jobm[0])."', 'R', '".date("m/d/Y H:i")."', '".$oper_num."', '".$emp_code."', '".$shift."', '".$pay_rate."', '".$pr_rate."', '".$job_rate."', '".$start_time."', '".$wc."');";
		$rs1 = $cSql->IsUpDel($conn, $sql);
		
		$sql = "insert into jobtran_mst (site_ref, job, trans_type, trans_date, oper_num, Uf_mc, start_time, wc, RESID) values ('".$site_ref."', '".sprintf("%' 10s",$jobm[0])."', 'C', '".date("m/d/Y H:i")."', '".$oper_num."', '".$resnum."', '".$start_time."', '".$wc."', '".$resnum."');";
		$rs1 = $cSql->IsUpDel($conn, $sql);
